...
(56 d'Hondt seats, 7 per region) for the 2011, 2016, 2021 and 2026 elections.
...
Data files are expected in old_data/files/holyrood/:
    ...
    holyrood-2026.json    — 2026 constituency results
    ...
    holyrood-2026-list.json  — 2026 regional list vote totals + seat allocation

The 2026 files are generated by scripts/scrape_holyrood_2026.py rather than
hand-maintained.
...
ELECTION_SPECS: tuple[ElectionSpec, ...] = (
    ...
    # 2026 is the first election fought on the new boundaries, so it lives on
    # the 2026 map alongside the remapped 2021 baseline below rather than on
    # the 2021 map the earlier elections use.
    ElectionSpec(
        year=2026,
        name="2026 Scottish Parliament Election",
        constituency_file="holyrood-2026.json",
        list_file="holyrood-2026-list.json",
        election_date="2026-05-07",
        comparison_election_name="2021 Scottish Parliament Election (2026 Boundaries)",
...
def import_constituency_results(
    ...
    if not data_file.exists():
        hint = (
            "Generate it with scripts/scrape_holyrood_2026.py."
            if spec.year == 2026
            else "This file is committed to the repository — check it out."
        )
        raise FileNotFoundError(f"Constituency data file not found: {data_file}\n{hint}")
    ...
    for seat_name, seat_payload in election_data.items():
        ...
        # Only write an electorate we actually have. Maps are shared between
        # elections (map 12 carries both 2026 and the remapped 2021 baseline),
        # so writing None for a source file that omits the figure would wipe a
        # value another election had supplied.
        if not dry_run and electorate is not None:
            db.set_seat_electorate(seat.id, int(electorate))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        action="append",
        choices=[2011, 2016, 2021, 2026],  # applies to base specs; remapped specs also filtered by year
