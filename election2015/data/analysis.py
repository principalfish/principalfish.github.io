...
class Seat(object):
    ...
    def writetofile(self):
      ...
      with open("info.csv", "ab") as output:
...
def revertseatpollpercentage(poll, party):
    a = date(2015, 5, 7) #election day
    b = date.today()
    daystoelection = (a - b).days
    ...
    change *= reversionfactor
    seats[poll].partyinfo[party][3] -= change
...
for poll in seatpolling:
    ...
    for key, val in vars(seatpolling[poll]).iteritems():
        if key in parties:
            ...
            revertseatpollpercentage(poll, key)
...
def writenationaltotals():
    with open("regions/projectionvotetotals.csv", "wb") as output:
...
def writeengland():
    ...
    output = open("regions/england.csv", "wb")
    writetofile = csv.writer(output, delimiter = ",")
    writetofile.writerow(headers)
...
def writegreatbritain():
    ...
    output = open("regions/greatbritain.csv", "wb")
    writetofile = csv.writer(output, delimiter = ",")
    writetofile.writerow(headers)
