...
from pollinganalysis import regionalpartytotals
...
for area in regionalpartytotals:
    region = regionalpartytotals[area][0]


    info = Region(alliance = region["alliance"],
        conservative = region["conservative"], dup = region["dup"],
        green = region["green"], labour = region["labour"],
        libdems = region["libdems"], other = region["other"],
        plaidcymru = region["plaidcymru"], sdlp = region["sdlp"],
        sinnfein = region["sinnfein"], snp = region["snp"],
        ukip = region["ukip"], uu = region["uu"])

    regionalpolling[area] = info
...
class Seat(object):
    ...
    def __init__(self, id, incumbent, seat, area, turnout2010, alliance, conservative, dup, green, labour, libdems, other, plaidcymru, sdlp, sinnfein, snp, ukip, uu, party, change):
    ...
    def smoothvotes(self, party, partypercent, seatrelativetoarea, arearelativechange, areanumericalchange):
        ...
        #     return new
            ...
            # for small parties, apply incumbency factor based on overall net change for that party
    ...
    def writetofile(self):
      ...
      with open("nowcast.csv", "ab") as output:
          ...
          csvwriter.writerow([self.id, self.seat, self.area, self.incumbent, self.partyinfo["conservative"][6],
                                ...
                                self.change, self.party, self.majority])
...
for poll in seatpolling:
    ...
    for key, val in vars(seatpolling[poll]).iteritems():
        if key in parties:
            if val == "":
                seats[poll].partyinfo[key][3] = 0
            else:
                seats[poll].partyinfo[key][3] = int(val)
...
def writenationaltotals():
    with open("nowcastregions/nowcastvotetotals.csv", "wb") as output:
    ...
    #    totalpercent = 0
        ...
        partytotals = [date.today() , datetime.time(datetime.now())] # for daily change in model
        ...
        #    totalpercent += votepercent # to make 100 for total % - pointless really
            ...
            partytotals.append(seatssum) # add party seat total to daily model tracker
            partytotals.append(votepercent)
    ...
    # with open("modelchange.csv", "ab") as updatefile:
    #     writetofile = csv.writer(updatefile, delimiter =",")
    #     writetofile.writerow(partytotals)
    #     updatefile.close()
...
def writeengland():
    ...
    output = open("nowcastregions/england.csv", "wb")
    writetofile = csv.writer(output, delimiter = ",")
    writetofile.writerow(headers)
...
def writegreatbritain():
    ...
    output = open("nowcastregions/greatbritain.csv", "wb")
    writetofile = csv.writer(output, delimiter = ",")
    writetofile.writerow(headers)
...
def writeregionaltotals():
    ...
    for region in regionalpolling:
        ...
        file = "nowcastregions/" + fileregion + ".csv"
        output = open(file, "wb")
        writetofile = csv.writer(output, delimiter =",")
        writetofile.writerow(headers)
