 ...
 * for maps whose mapMode sets `seatComparison: false` (aggregate-only comparison, for maps
 * whose seats are redrawn between cycles; no current map sets it). The aggregate vote-totals
 * delta columns are unaffected — they key off comparison-data presence, not this flag.
 * Absent flag ⇒ shown.
 *
 * Predict view is exempt from the `seatComparison: false` suppression: a projection always runs
 * against a same-cycle baseline on the same map (identical seats), so per-seat gains are
 * meaningful there even for maps that disable them for the cross-boundary historical
 * comparison. Referendums stay suppressed regardless.
 * @returns {boolean}
 */
...
class AppState {
  constructor() {
    ...
    /** Referendum choropleth config `{highParty, lowParty, threshold, metricLabel}` when the
     * current election declares a `referendum` block, else null. Drives the default
     * two-party (e.g. Leave/Remain) map; see `setupChoroplethConfig`. */
    this.referendumConfig = null;
  ...
  async init(view, parliament, requestedId) {
    ...
    this.referendumConfig = (this.currentElection.referendum && typeof this.currentElection.referendum === 'object')
      ? this.currentElection.referendum
      : null;
  ...
  buildChoroplethConfig() {
    ...
    // Referendum choropleth params, manifest-driven via the election's `referendum` block.
    // Defaults preserve the legacy EU-referendum behaviour when the block is absent/partial
    // (e.g. an election that still sets `referendum: true`).
    const refCfg = this.referendumConfig || {};
    const refHighParty = refCfg.highParty || 'leave';
    const refLowParty = refCfg.lowParty || 'remain';
    const refThreshold = Number.isFinite(refCfg.threshold) ? refCfg.threshold : 50;
    const refMetricLabel = refCfg.metricLabel || 'Leave vote share';
    ...
    this.electionData.currentSeats.forEach((seat) => {
      ...
      if (isReferendumTypeWithDefaults) {
        value = Seat.voteSharePct(seat, refHighParty);
      } else {
    ...
    switch (kind) {
      case 'referendum': {
        // Three-point scale anchored at the configured threshold (50% for EU). Low high-party
        // share maps to the low-party colour, high share to the high-party colour. Both colours
        // come from the manifest so they stay in sync with the party list. Party pair, threshold,
        ...
        config.legend = {
          isDelta: true,
          title: refMetricLabel,
          startColour: lowColour,
          midColour: neutralColour,
          endColour: highColour,
          minLabel: `${formatPct(minValue)}%`,
          midLabel: `${refThreshold}%`,
 ...
 * params as appropriate — the inverse of the URL parsing in {@link AppState#init}. The
 * predict payload is sourced from `state.predictModel.serialize()` when view==='predict'.
 * Lives here (rather than in the orchestrator) so both electionmaps.js and the predict
 * controller can serialise routes without a circular import.
 * @param {string} view - View name to set ('election' | 'predict' | 'polltracker').
 * @returns {URLSearchParams}
 */
export function buildRouteSearchParams(view) {
